 ...
 *
 * Test Phases:
 * 1. Phase 1 (TP_DCLS_INT_FLOP / Case 201): Program Counter Flop Corruption.
 *    Forces 1-bit corruption in subordinate Program Counter (ifu_pc_ff). Asserts mismatch
 *    detection within 1 to 2 clock cycles.
 * 2. Phase 2 (TP_DCLS_INT_GPR / Case 200): General Purpose Register (GPR) Bit-Flip.
 *    Performs arithmetic on a5 (x15). Flips bit 15 of subordinate GPR bank and verifies
 *    divergence when read into the ALU execution stage.
 * 3. Phase 3 (TP_DCLS_INT_CSR / Case 202): Architectural CSR Flop Tamper.
...
int main () {
    ...
    *threshold = 1;
    ...
    if (test_count == 0) {
        // Phase 1 (TP_DCLS_INT_FLOP): Program Counter Flop Corruption
        printf("Phase 1 (TP_DCLS_INT_FLOP): Forcing 1-bit corruption in Program Counter flop (case 201)...\n");
        test_count = 1;
        tohost = (201 << 8) | CMD_INJ_LOCKSTEP;
        for (volatile int i = 0; i < 500; i++) asm volatile ("nop");
        printf("FAIL: Phase 1 did not trap!\n");
        tohost = 1;
        ...
        // Phase 2 (TP_DCLS_INT_GPR): GPR Bit-Flip on a5 (x15)
        printf("Phase 2 (TP_DCLS_INT_GPR): Forcing bit 15 flip in Lockstep GPR a5 (case 200)...\n");
        test_count = 2;
        ...
        printf("FAIL: Phase 2 did not trap!\n");
        tohost = 1;
        ...
        // Phase 3 (TP_DCLS_INT_CSR): Architectural CSR Flop Tamper (mtvec)
        printf("Phase 3 (TP_DCLS_INT_CSR): Forcing bit-flip inside Lockstep mtvec CSR flop (case 202)...\n");
        test_count = 3;
        ...
        printf("FAIL: Phase 3 did not trap!\n");
        tohost = 1;
        ...
        printf("All 3 internal state tamper phases trapped and passed verification!\n");
        mie = read_csr(mie);
        mie &= ~(1 << 11);
        write_csr(mie, mie);
