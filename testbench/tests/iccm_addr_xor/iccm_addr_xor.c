 ...
 * 5. Access (ME/clken) Fault Injection (0xE7): Suppressed memory enable prevents valid instruction retrieval.
 */
...
// Requires RV_ICCM_ADDR_XOR=1 in the hardware build (-set iccm_addr_xor=1).
// When enabled in veer.config, RV_ICCM_ADDR_XOR is emitted to defines.h.
#if !defined(RV_ICCM_ADDR_XOR) || (RV_ICCM_ADDR_XOR != 1)
#error "Configuration Mismatch: RV_ICCM_ADDR_XOR is not set to 1! Hardware ICCM address-XOR infection must be enabled (-set iccm_addr_xor=1)."
#endif
...
#define INJECT_ICCM_ACCESS_FAULT 0xE7
...
void trap_handler(void) {
    ...
    // Disable any active fault injection (redundant with crt0.s, for symmetry)
...
int main(void) {
    ...
    if (boot_phase == 1) {
        ...
        // -------------------------------------------------------------
        // Phase 4: ICCM Access (ME/clken) Fault Injection (0xE7)
        // -------------------------------------------------------------
        ...
        printf("[Phase 4] Testing ICCM Access (ME/clken) Fault Injection (0xE7)...\n");

        copy_code_to_iccm_dest(ICCM_BASE + 0x400);

        void (*func_access_fault)(void) = (void (*)(void))(ICCM_BASE + 0x400);
        putchar(INJECT_ICCM_ACCESS_FAULT);
        printf("[Phase 4] Executing from 0x%x with access fault (expecting ECC trap)...\n", (uint32_t)(ICCM_BASE + 0x400));
        func_access_fault();

        putchar(DISABLE_ICCM_FAULT);
        printf("FAIL [Phase 4]: Did not hit expected ECC trap on ICCM access fault!\n");
        putchar(TEST_FAILED);
        return 1;
