...
class BaseScoreboard(uvm_component):
    ...
    def check_phase(self):
        ...
        if self.port.can_get():
            self.passed = True

        while self.port.can_get():
            _, item = self.port.try_get()

            if item.signals["rst_ni"] == 0:
                pred_count = 0

            else:

                curr_count = int(item.signals["cnt_o"])

                # Align the prediction with the observation on first sample
                if pred_count is None:
                    pred_count = curr_count

                # On critical fault the counter state gets corrupted. So
                # align the prediction with observation as well
                if item.signals["ecc_fatal_o"]:
                    pred_count = curr_count

                # A mismatch
                if pred_count != curr_count:
                    self.logger.error(
                        f"cnt_o mismatch! actual:{curr_count}, predicted:{pred_count}"
                    )
                    self.passed = False

                # Predict counter state based on control signals
                if item.signals["clr_i"]:
                    pred_count = 0
                else:
                    if item.signals["inc_i"] and not item.signals["dec_i"]:
                        pred_count += 1
                    if item.signals["dec_i"] and not item.signals["inc_i"]:
                        pred_count -= 1
...
class BaseTest(uvm_test):
    ...
    def __init__(self, name, parent, seq_class, scb_class=BaseScoreboard):
        super().__init__(name, parent)
    ...
    async def run_phase(self):
        ...
        cocotb.top.inc_i.value = 0
        cocotb.top.dec_i.value = 0
        cocotb.top.clr_i.value = 0
