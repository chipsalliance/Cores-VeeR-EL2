...
class BaseEnv(uvm_env):
    ...
    def build_phase(self):
        ...
        self.fault_monitor = FaultMonitor(
            ...
            clock_domain=self.clock_domain,
            signals=[
                cocotb.top.exec_fault_q[0],
                cocotb.top.exec_fault_q[1],
                cocotb.top.exec_fault_q[2],
            ],
        )
...
class BaseTest(common.BaseTest):
    ...
    async def initial(self):
        ...
        # Initialize
        cocotb.top.exec_output_inhibit.value = MuBiFalse

        cocotb.top.exec_fault_d[0].value = MuBiFalse
        cocotb.top.exec_fault_d[1].value = MuBiFalse
        cocotb.top.exec_fault_d[2].value = MuBiFalse

        cocotb.top.exec_fault_clr[0].value = MuBiFalse
